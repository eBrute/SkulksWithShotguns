...
--
-- This mixin contains all the code and the common set of netvars. The full mixin (ModelMixin)
...
local Shared_PreparePhysics                     = Shared.PreparePhysics
...
local function Print(message)
    if Shared.GetDevMode() then Shared.Message(message) end
end
...
local function SynchronizeAnimation(self, syncNodesOnServer)
    ...
    if graph ~= nil then
        ...
        -- if the animation graph has changed its loadId (hotloading) then reset the animation state
        local loadId = graph:GetLoadId()
        self.animationGraphLoadId = self.animationGraphLoadId or loadId
        if self.animationGraphLoadId ~= loadId then
            self.animationGraphLoadId = loadId
            ResetAnimationState(self)
        end

        local state = self.animationState
        AnimationGraphState_PrepareForGraph(state, graph)
        if self.animationSequence == nil then return end
...
function BaseModelMixin:GetIsHighlightEnabled()

    if self.SetIsHighlightEnabled then
        return self:SetIsHighlightEnabled()
    end
...
local function SetHighlight(self)
    ...
    if self:GetIsHighlightEnabled() == 1 then
        if self:isa("Gorge") or self:isa("Babbler") then
            self._renderModel:SetMaterialParameter("highlight", 1)
        elseif HasMixin(self, "Team") and self:GetTeamNumber() == kAlienTeamType then
...
local function UpdateRenderModel(self)
    ...
    if renderModel ~= nil and allowUpdates then
        ...
        -- when we cull entities on the client to avoid doing unnecessary physics, sometimes the model can be left
        -- behind at its original position and won't get updated until the player is available again. By tracking
        -- when we last did a physics update, we detect that we didn't do it this time around, and then we update
        -- the model origin to the current position
        -- We only do this for non-parented entities
        if self:GetParentId() == Entity.invalidId and self.lastPhysicsUpdateTime ~= Shared.GetTime() then
            --  Log("%s: shifting model to user position to avoid culling effects (physTime %s vs %s)", self, self.lastPhysicsUpdateTime, Shared.GetTime())
            self._modelCoords.origin = self:GetOrigin()
        end
...
-- Initialize fields (ClientModelMixin initializes in Client, ModelMixin only initializes in Server)
function BaseModelMixin:InitializeFields()
...
function BaseModelMixin:MarkPhysicsDirty()
    ...
    if desiredModelIndex and desiredModelIndex > 0 then -- don't do anything if model index is invalid (ie not received from server yet)
...
function BaseModelMixin:OnUpdate(deltaTime)
    ...
    -- Prevent floating stuff caused by late calls to SetModel()
    if Client then
...
function BaseModelMixinDebugSpeed(self)
    if Client and self:isa("Marine") then
        local renderOrigin = self:GetOrigin()
        local renderTime = Shared.GetTime()
        if self.prevRenderOrigin then
            local diff = renderOrigin - self.prevRenderOrigin
            local dt = renderTime - self.prevRenderTime
            local speed = diff:GetLength() / dt
            Log("%s: speed %s, diff %s, dt %s", self, speed, diff, dt)
        end
        self.prevRenderOrigin = renderOrigin
        self.prevRenderTime = renderTime
    end
end
...
-- support for physics update parallellism (not for the server)
if Client then
    ...
    function BaseModelMixin:OnPreparePhysics()
        ...
        if model ~= nil then
            -- a "dynamic" model has its bone coordinates determined by its physicsModel (ie, ragdolls), while
            -- other models have their physics determined by their model/animation
            -- The DynamicServer type is treated as Dynamic on the client.
            local dynamic = self.physicsType == PhysicsType.Dynamic
            Shared_PreparePhysics(self, self.animationState, model, self.poseParams, self.boneCoords, dynamic, self._modelCoords, self.physicsModel)
        end
...
function BaseModelMixin:LogAnimationState()
    ...
    if graph and model then
        ...
        Print(string.format("model = %s, graph = %s", self:GetModelName(), graph:GetFileName()))
...
local function SetBoneUpdatingEnabled(enable)
    ...
    if _enableBoneUpdating then
        Print("Bone updating enabled")
    else
        Print("Bone updating disabled")
    end
...
local function SetPoseParamsEnabled(enable)
    ...
    if _enablePoseParams then
        Print("Pose params enabled")
    else
        Print("Pose params disabled")
    end
...
-- By default, all models allows wallwalking on them. Override and return false to forbid.
function BaseModelMixin:GetIsWallWalkingAllowed()
    return true
end
